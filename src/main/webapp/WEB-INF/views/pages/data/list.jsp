...
<body>
<form id="searchFrm" action="<c:url value="/data/search"/>" class="form-horizontal" method="post">
<section class="content-header">
  ...
  <ol class="breadcrumb">
    ...
    <li ><a href="<c:url value="/data/list"/>">데이터 관리</a></li>
    <li ><a href="<c:url value="/data/list"/>">데이터 관리</a></li>
  </ol>
</section>
        ...
        <div class="col-md-12">
          <div class="bottom-25">
            ...
            <div class="custom-tab-parent">
                <div class="active">내부 센서데이터 보기</div>
                <div>내부 센서그래프 보기</div>
                <div>외부 센서데이터 보기</div>
                <div>외부 센서그래프 보기</div>
                <div>영농일지</div>
                      ...
                      <c:if test="${not empty rows}">
                        <c:forEach items="${rows}" var="rows" varStatus="idx">
                          ...
                          <tr>
                            ...
                            <td >${rows.temperature1}</td>
                            <td >${rows.temperature2}</td>
                            <td >${rows.temperature3}</td>
                            <td >${rows.temperature4}</td>
                            <td >${rows.temperature5}</td>
                            <td >${rows.temperature6}</td>
                            <td >${rows.temperature7}</td>
                            <td >${rows.temperature8}</td>

                            <td >${rows.humidity1}</td>
                            <td >${rows.humidity2}</td>
                            <td >${rows.humidity3}</td>
                            <td >${rows.humidity4}</td>
                            <td >${rows.humidity5}</td>
                            <td >${rows.humidity6}</td>
                            <td >${rows.humidity7}</td>
                            <td >${rows.humidity8}</td>

                            <td >${rows.carbondioxide1}</td>
                            <td >${rows.carbondioxide2}</td>
                            <td >${rows.carbondioxide3}</td>
                            <td >${rows.carbondioxide4}</td>
                            <td >${rows.carbondioxide5}</td>
                            <td >${rows.carbondioxide6}</td>
                            <td >${rows.carbondioxide7}</td>
                            <td >${rows.carbondioxide8}</td>
                          </tr>
                        </c:forEach>
                      </c:if>
...
//      console.log(this);
    ...
    $('#searchBtn').click(function(){
      $('#searchFrm').attr('action','/data/search');
